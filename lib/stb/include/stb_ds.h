  ...
  Dynamic Arrays
    ...
    arrclear:
        void arrsetlen(T* a, int n);
          Changes the length of the array to 0.
  ...
  Hash maps & String hash maps
    ...
    Function interface (actually macros):
      ...
      shputs
        T hmputs(T*, T item)
        T shputs(T*, T item)
          Inserts a struct with T.key and T.value into the hashmap. If the
struct is already present in the hashmap, updates it.
...
"functions" write to the provided pointer. This means: (a) the pointer must be
an lvalue, and (b) the pointer to the data structure is not stable, and you must
maintain it the same as you would a realloc'd pointer. For example, if you pass
a pointer to a dynamic array to a function which updates it, the function must
return back the new pointer to the caller. This is the price of trying to do
this in C.
...
you're using C99 and later in GCC or clang, or if you're using C++ in GCC.
